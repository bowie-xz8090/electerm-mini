...
export default Store => {
  ...
  Store.prototype.runCommandInTerminal = function (cmd) {
    window.store.batchInputSelectedTabIds.forEach(id => {
      const term = refs.get('term-' + id)
      if (!term) {
        return
      }
      term.clearShellInputLine?.()
      setTimeout(() => {
        term?.runQuickCommand?.(cmd)
      }, 60)
    })
  }

  Store.prototype.getSmartShellHistory = function (tabId = window.store.activeTabId) {
    const tab = window.store.tabs.find(t => t.id === tabId)
    return Array.isArray(tab?.smartShellHistory) ? tab.smartShellHistory : []
  }

  Store.prototype.setSmartShellHistory = action(function (tabId, history) {
    const tab = window.store.tabs.find(t => t.id === tabId)
    if (!tab) {
      return
    }
    tab.smartShellHistory = Array.isArray(history)
      ? history.slice(-18)
      : []
  })

  Store.prototype.appendSmartShellHistory = action(function (tabId, entry) {
    const tab = window.store.tabs.find(t => t.id === tabId)
    if (!tab) {
      return null
    }
    const history = Array.isArray(tab.smartShellHistory)
      ? tab.smartShellHistory.slice()
      : []
    const nextEntry = {
      id: entry?.id || uid(),
    ...
    tab.smartShellHistory = history.slice(-18)
    return nextEntry
  })

  Store.prototype.updateSmartShellHistory = action(function (tabId, entryId, updates) {
    const tab = window.store.tabs.find(t => t.id === tabId)
    if (!tab || !Array.isArray(tab.smartShellHistory) || !entryId) {
      return null
    }
    const history = tab.smartShellHistory.map(item => {
      if (item.id !== entryId) {
        return item
      }
      return {
    ...
    tab.smartShellHistory = history
    return history.find(item => item.id === entryId) || null
  })
  ...
  Store.prototype.aiConfigMissing = function () {
    return aiConfigsArr
      .filter(k => k !== 'proxyAI' && k !== 'nameAI')
      .some(k => !String(window.store.config[k] || '').trim())
  }
